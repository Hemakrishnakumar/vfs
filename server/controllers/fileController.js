...
export const updateFile = async (req, res, next) => {
  ...
  const fileData = await File.findOneAndUpdate({ _id: id, userId: req.user._id }, { $set: { name: newFilename } });
  if (!fileData)
...
export const deleteFile = async (req, res, next) => {
  const { id } = req.params;
  const file = await File.findOneAndDelete({ _id: id, userId: req.user._id }).lean();
  if (!file)
